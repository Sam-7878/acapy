...
# Scenario별 워크로드 함수
# ─────────────────────────────────────────────────────────
def scenario1_realtime_turntaking(cur, conn, cfg, params, nodes, depths, iterations):
...
def scenario2_chain_churn(cur, conn, cfg, params, nodes, depths, iterations):
...
def scenario3_partition_reconciliation(cur, conn, cfg, params, nodes, depths, iterations):
    ...
    while time.time() - start < split_dur:
        ...
        for did in range(boundary, total):
            cur.execute("UPDATE delegation SET hq_id=%s WHERE drone_id=%s", (cfg.headquarters_id, did))
        conn.commit()
    print(f"Partition 해제, 동기화 {params['partition_reconciliation']['post_reconcile_sync_requests']}건, duration={recon_dur}s --")
    start2 = time.time()
    while time.time() - start2 < recon_dur:
        query = get_bench_query(cfg.headquarters_id, depths[0])
        _ = benchmark_query(cur, query, params['partition_reconciliation']['post_reconcile_sync_requests'])
    print("Reconciliation benchmark 완료.")
...
if __name__ == '__main__':
    ...
    if args.scenario == '1':
        ...
        scenario1_realtime_turntaking(cur, conn, cfg, params, scale_up_nodes, depths, iterations)
    elif args.scenario == '2':
        print("=== Running Scenario A-2: Chain-Churn ===")
        scenario2_chain_churn(cur, conn, cfg, params, scale_up_nodes, depths, iterations)
    else:
        print("=== Running Scenario A-3: Partition & Reconciliation ===")
        scenario3_partition_reconciliation(cur, conn, cfg, params, scale_up_nodes, depths, iterations)
